...
void verifyGrid(struct Grid &grid, int min, int max, int expectedNode)
{
    int x, z, y;
    // -x  +x 
    for (y = min; y <= max; y++)
...
void resetGridNodes(struct Grid &grid, int min, int max, int newNode)
{
    int x, z, y;
    // -x  +x 
    for (y = min; y <= max; y++)
...
void verifyInitalClearGridField(struct Grid &grid)
{
    int x, y, z;
...
public:
    ...
    void SetUp() {
        ...
        std::vector<struct BoundingBox> boxes = Partition::getProcessBoxes(1, nx, nx, nz);
        gridCPU = new GridKernelCPU(boxes[0], trans, "D3Q27");
...
public:
    ...
    void SetUp() {
        ...
        gridCPU = new GridKernelCPU(boxes[0], trans, "D3Q7");
    }
...
TEST_F(GridKernelCubeIsBetweenNodesTest, calculateQ_checkifQIsInFSolidNode_whenTriangleIsStraight_forD3Q7){
    ...
    //struct Vertex p1 = getVertex(3.5, 2, 2);
    //struct Vertex p2 = getVertex(3.5, 2, 9);
    //struct Vertex p3 = getVertex(5.5, 9, 9);

    //struct Vertex edge1, edge2, normal;
    //edge1 = minus(p2, p1);
    //edge2 = minus(p3, p2);
    //normal = crossProduct(edge1, edge2);

    //struct Triangle t = Triangle(p1,p2,p3,normal);
    ...
    //struct VertexInteger v = getVertexInt(3, 5, 5);
    //Distributions7 d;
    //int dirs = 7;
