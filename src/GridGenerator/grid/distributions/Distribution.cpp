...
struct Distribution DistributionHelper::getDistribution7() {

    struct Distribution d7;
    d7.name = "D3Q7";
...
struct Distribution DistributionHelper::getDistribution13() {
    struct Distribution d13;
    d13.name = "D3Q13";
...
struct Distribution DistributionHelper::getDistribution19() {
    struct Distribution d19;
    d19.name = "D3Q19";
...
struct Distribution DistributionHelper::getDistribution27() {
    struct Distribution d27;
    d27.name = "D3Q27";
...
struct Distribution DistributionHelper::getDistribution(std::string name)
{
    if (name == "D3Q7")
...
std::vector<std::vector<doubflo> > DistributionHelper::getQsWithoutRowsWithOnlyZeroValues(const struct Grid &grid, const struct Distribution &d) {
    return getVectorWithoutRowsWithOnlyZeroValues(getAllQsOnFluidNodes(grid, d));
}

std::vector<std::vector<doubflo> > DistributionHelper::getAllQsOnFluidNodes(const struct Grid &grid, const struct Distribution &d)
{
...
int DistributionHelper::getNeighborNodeIndexInGivenDirection(const struct Distribution &d, const struct Grid &grid, const int node, const int dir_index)
{
    struct Vertex dir = Vertex((doubflo)d.dirs[dir_index * DIMENSION + 0], (doubflo)d.dirs[dir_index * DIMENSION + 1], (doubflo)d.dirs[dir_index * DIMENSION + 2]);
    int x, y, z;
    grid.transIndexToCoords(node, x, y, z);
    struct Vertex solid_node = Vertex((doubflo)x, (doubflo)y,(doubflo)z);
    struct Vertex fluid_node = Vertex(solid_node.x - dir.x, solid_node.y - dir.y, solid_node.z - dir.z);
